...
def find_chimera(alns, aligner, bam, min_coverage=0.95, check_alt_paths=False, max_splits=3, debug=False):
	...
	if len(primary_aligns) > max_splits:
	    if debug:
		sys.stdout.write('%s number of primary split alignments(%d) exceeds maximum:%d\n' % (primary_aligns[0].query, len(primary_aligns), max_splits))
	    return None, None
	...
	if primary_paths:
		...
		if align:
		    if debug:
			sys.stdout.write('filter out chimera %s(%sbp) because of end-to-end secondary alignment %s:%s-%s %s\n' % (align.query,
			                                                                                                          align.query_len,
			                                                                                                          align.target,
			                                                                                                          align.tstart,
			                                                                                                          align.tend,
		...
		replaced_chimera = _replace_trl(primary_aligns, secondary_aligns)
		if replaced_chimera:
		    if debug:
			sys.stdout.write('replaced %s trl %s:%s-%s %s:%s-%s with %s:%s-%s %s:%s-%s\n' % (alns[0].qname,
		...
		for i in range(len(primary_chimera)):
			...
			if debug:
			    sys.stdout.write('%s primary alignment %s:%s-%s is dubious, can be replaced by %d secondary alignments\n' % (primary_chimera[i].query,
		...
		for indices in secondary_paths:
		    ...
		    if _is_secondary_chimera_better(primary_chimera, secondary_chimera):
			if debug:
			    sys.stdout.write('Filter out %s as alternative chimera can be formed in seconday alignments\n' % alns[0].qname)
			passed = False
			break
	...
	for i in (0, 1):
		...
		if alt_alns[i]:
			...
			if int(alt_aln.opt('NM')) - int(matched[i].opt('NM')) <= ambiguous_NM:
			    if debug:
				sys.stdout.write('ambigous subseq aln: %s %s %s %s NM:%s\n' % (alt_aln.qname, 
		...
		if debug:
		    sys.stdout.write('Failed to map subseq end-to-end %s:%s-%d %s:%s-%s\n' % (adj_aligns[i].query, 
		                                                                              adj_aligns[i].qstart, 
		                                                                              adj_aligns[i].qend, 
		                                                                              adj_aligns[i].target,
		                                                                              adj_aligns[i].tstart,
...
def call_event(align1, align2, homol_seq='-', homol_coords=(), novel_seq='-', contig_seq=None, no_sort=False, probe_side_len=25, debug=False):
	...
	# use tuple for 'chroms', 'breaks', 'orients': don't want them to be changed (immutable)
