...
def find_support(
    ...
    def extract_support_spans(event):
        seq_ids = []
        support_spans = []
        if ',' not in event.seq_id and isinstance(event.support_span, tuple):
            seq_ids.append(event.seq_id)
            support_spans.append(event.support_span)
        elif isinstance(event.support_span, str):
            if len(event.support_span) > 0:
                ...
                if len(seq_ids) != len(spans):
                    print('unmatched seq_ids(%s) and support_spans(%s)'.format(event.seq_id, event.support_span))
                    return None, None
                else:
                    support_spans = [list(map(int, span.split('-'))) for span in spans]
            else:
                return None, None
    ...
    def assign(event_to_coords, support):
        for event in event_to_coords:
            ...
            if event.event in events_flanking:
                ...
                total_flanking = None
            if event in event_to_coords:
                for seq_id, span in event_to_coords[event]:
                    if seq_id not in support:
                        if debug:
    ...
    for event in events:
        ...
        for seq_id, support_span in zip(seq_ids, support_spans):
            ...
            event_to_coords[event].append(
                (seq_id, '-'.join(map(str, [span[0], span[1]]))))
    ...
    for read in [r for r in reads if not r.is_unmapped]:
        ...
        if frag is not None and\
           ((frag[0] >= breaks[0] and frag[0] <= breaks[1]) or
            ...
            # if frag is not None and frag[0] <= breaks[0] - overlap_buffer and
            # frag[1] >= breaks[1] + overlap_buffer:
            uniq_frags.add(frag)
            if debug:
                sys.stdout.write(
                    "Accepted flanking pair: %s %s %s %s %s\n" %
...
def find_spanning(
    ...
    break_seq = contig_seq[breaks[0] - 1 -
                           overlap_buffer: breaks[1] + overlap_buffer]
        ...
        if read.alen:
            if read.pos < breaks[0] - overlap_buffer and\
               ...
               is_fully_mapped(read, contig_len, perfect=perfect):
                ...
                if key not in pos and read.qname not in names:
                    pos.add(key)
                    ...
                    if debug:
                        sys.stdout.write(
                            ...
                            (perfect, read.qname, breaks, (read.pos + 1, read.pos + read.alen), read.seq, strand))
...
def is_fully_mapped(read, contig_len, perfect=False):
    ...
    """
    # take out rlen==alen condition to allow for insertions and deletion in
    # alignments
    if not re.search(
            '[HS]',
    ...
    for key, group in groupby(bam.fetch(until_eof=True), lambda x: x.tid):
        ...
        for event, breaks in coords[contig]:
            ...
            if event in events_flanking:
                results[breaks]['flanking'], tlens = find_flanking(
                    reads, breaks_sorted, contig_len, overlap_buffer=overlap_buffer, debug=debug)
        ...
        for breaks in results.keys():
            ...
            support.append(
                (contig,
                 breaks[0],
                    breaks[1],
                    results[breaks]['spanning'],
                    results[breaks]['flanking']))
...
def create_batches(
    ...
    if size == 0:
        ...
        for i in range(0, len(contigs), size):
...
def fetch_support(
    ...
    results = {}
    tlens_all = []
    for contig, event_spans in coords.items():
        results[contig] = {}
        contig_seq = contig_fasta.fetch(contig)
        contig_len = len(contig_seq)
        for event, span in event_spans:
            ...
            if event in events_flanking:
                flanking, tlens = find_flanking(
                    reads, span_sorted, contig_len, overlap_buffer=overlap_buffer, debug=debug)
