...
class SVFinder:
    ...
    def filter_probes(cls, events, genome_index_dir, genome_index, working_dir, probe_length, debug=False):
        ...
        def parse_and_filter(bam, fasta_file, qname_to_event, indel_size_check=20):
            ...
            events_by_key = dict((events[i].key(), i)
                ...
                if aln.is_unmapped:
                    ...
                    if not bad:
                        if event_type in ('fusion', 'read_through'):
                            failed = within_same_gene(
                                alns, fasta.fetch(aln.query_name), event)
                            if failed:
    ...
    def filter_subseqs(cls, events, query_fa, genome_index_dir, genome_index, working_dir, subseq_len=None, debug=False):
        ...
        def create_query_fasta(events, fa_file, min_size=20):
            ...
            for event in events:
                ...
                subseqs = event.get_subseqs(
                    query_fa.fetch(seq_id), seq_breaks=seq_breaks)
        ...
        def parse_and_filter(bam, sizes, window=100):
            ...
            for i in range(len(events)):
                # not always the first contig is chosen because of introductin
                # of pick_sub_seqs()
            ...
            for query, group in groupby(bam.fetch(until_eof=True), lambda aln: aln.query_name):
                ...
                if mapped_alns:
                    ...
                    matched = []
                    if seq_id not in subseq_mappings:
                        subseq_mappings[seq_id] = {}
                    if key not in subseq_mappings[seq_id]:
    ...
    def update_adj(self, adj, aligns, query_seq, target_type, block_matches=None):
        ...
        if adj.transcripts:
            ...
            # reverse-complement becasue sequences originally identified are in
        ...
        adj.set_probe(query_seq, len_on_each_side=int(self.probe_len / 2))
    ...
    def map_transcripts_to_adj(self, adj, aligns, block_matches=None, genes=(None, None)):
        ...
        def pick_best(chrom, breakpoint, orient, block_matches, gene):
            def cmp_mapping(t1, t2):
                return compare_mapping(metrics[t1], metrics[t2])

            metrics = {}
            for tid, matches in block_matches.items():
        ...
        if event.exon_bounds and event.transcripts:
            if (event.exon_bounds[0] is False and event.exon_bounds[1] is True) or\
               (event.exon_bounds[0] is True and event.exon_bounds[1] is False):
                ...
                if matched_exon is None or unmatched_exon is None:
                    return
    ...
    def find_indels(self, align, query_fasta, target_fasta, target_type,
        ...
        if not re.search(r'[IDN]\d+', align.cigarstring):
            return adjs
        ...
        for i, j in extract_flanking_blocks():
            ...
            if target_type == 'genome' and event_type == 'del' and is_del_possibly_intron(
                    target_breaks):
                continue
        ...
        if matches_downstream:
            ...
            if matches[0][0] == 1:
                ...
                if len(matches) == 1:
                    ...
                    # more work needed, novel sequence may actually be more
                    # than one copy
    ...
    def is_repeat_number_change(self, adj, query_fasta, target_fasta, strand, max_size=30):
        ...
        if adj.rearrangement == 'ins':
            ...
            changed_seq = query_fasta.fetch(
                adj.seq_id, seq_breaks_sorted[0], seq_breaks_sorted[1] - 1)
            if strand == '-':
    ...
    def is_read_through_from_single_align(self, block_matches, align):
        ...
        for tid in block_matches.keys():
            if tid in self.transcripts_dict:
                genes.add(self.transcripts_dict[tid].gene)
