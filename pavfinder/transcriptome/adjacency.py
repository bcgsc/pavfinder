...
class Adjacency:
    ...
    def __init__(
            ...
            ins_seq=None,
            ins_seq_coords=None,
            copy_num_change=None,
            repeat_seq=None,
            upstream_transcript=None,
            downstream_transcript=None,
            exons_oriented=None,
            exon_bounds_oriented=None,
            feature=None,
            effect=None,
            in_frame=None,
            splice_motif=None,
        ...
        if not self.exons and not self.exon_bounds:
            ...
            if self.transcripts:
                if self.transcript_breaks:
                    for i in range(len(self.transcript_breaks)):
                        exons.append(
                            self.transcripts[i].txt_coord_to_exon(
                                self.transcript_breaks[i]))
                        exon_bound_edge = self.transcripts[i].at_exon_bound(
                            txt_coord=self.transcript_breaks[i], exon_num=exons[-1], return_edge=True)
                        exon_bounds.append(
                            check_exon_bound(
                                exon_bound_edge,
                                self.orients[i],
                    ...
                    for i in range(len(self.genome_breaks)):
                        exons.append(
                            self.transcripts[i].coord_to_exon(
                                self.genome_breaks[i]))
                        exon_bound_edge = self.transcripts[i].at_exon_bound(
                            genome_coord=self.genome_breaks[i], exon_num=exons[-1], return_edge=True)
                        exon_bounds.append(
                            check_exon_bound(
                                exon_bound_edge,
                                self.orients[i],
    ...
    def adjust_transcript_breaks(self, align_strands):
        ...
        if not self.exon_bounds[0]:
            for i in range(1, len(self.homol_seq) + 1):
                ...
                exon_bound_edge = self.transcripts[0].at_exon_bound(
                    txt_coord=tbreak, exon_num=exon_num, return_edge=True)
                ...
                if check_exon_bound(exon_bound_edge, self.orients[0]):
                    ...
                    if isinstance(homol_seq_coords, list):
        ...
        if self.homol_seq and not self.exon_bounds[1]:
            for i in range(1, len(self.homol_seq) + 1):
                ...
                exon_bound_edge = self.transcripts[1].at_exon_bound(
                    txt_coord=tbreak, exon_num=exon_num, return_edge=True)
    ...
    def get_size(self):
        ...
        if self.rearrangement == 'dup' or self.rearrangement == 'inv-dup':
            if self.transcript_breaks[0] is not None and self.transcript_breaks[1] is not None:
                ...
                size = breaks_sorted[1] - breaks_sorted[0] + \
                    1 + novel_seq_len - homol_seq_len
            ...
            if self.seq_breaks[0] is not None and self.seq_breaks[1] is not None:
                ...
                size = seq_breaks_sorted[1] - seq_breaks_sorted[0] - \
                    1 + novel_seq_len - homol_seq_len
            ...
            if self.target_breaks[0] is not None and self.target_breaks[1] is not None:
                ...
                size = target_breaks_sorted[1] - \
                    target_breaks_sorted[0] - 1 + homol_seq_len
    ...
    def report(self, event_id=None):
        data = []
        for item, label in self.report_items.items():
            value = 'na'
            if item == "name":
                ...
                if hasattr(self, attr):
                    ...
                    if (isinstance(values, tuple) or isinstance(
                            values, list)) and len(values) > int(index):
    ...
    def as_bedpe(self, event_id=None):
        data = []
        for item, label in self.bedpe_items.items():
            value = 'na'
            if item == "name":
                if event_id is not None:
                    value = event_id
    ...
    def reverse_genome_breaks(self, change=False, annotation=False):
        ...
        if not change:
            if None not in new_vals.values():
                returnable = []
                for field in fields:
    ...
    def key(self):
        ...
        for i in (0, 1):
            ...
            if self.orients and\
               ((isinstance(orients, tuple) or isinstance(orients, list)) and
                    len(orients) == 2):
    ...
    def set_probe(self, query_seq, len_on_each_side=50):
        ...
        if up_short > 0 and down_short > 0:
            ...
            self.probe = up_seq + mid_seq + \
                down_seq[:min(len_on_each_side + up_short, len(down_seq))]
        elif down_short > 0:
            self.probe = up_seq[-1 * min(len(up_seq),
                                         len_on_each_side + down_short)::] + mid_seq + down_seq
        else:
            self.probe = up_seq[-1 * min(len(up_seq), len_on_each_side)::] + \
                mid_seq + down_seq[:min(len_on_each_side, len(down_seq))]

    def get_subseqs(self, query_seq, len_on_each_side=None, seq_breaks=None):
        if not seq_breaks:
            if isinstance(self.seq_breaks, tuple):
                breaks = list(map(int, sorted(list(self.seq_breaks))))
        else:
            breaks = list(map(int, sorted(list(seq_breaks))))
        subseqs = []
        if breaks:
    ...
    def merge(cls, all_adjs):
        ...
        for key, adjs in merged.items():
            if len(adjs) == 1:
                ...
                if adjs[0].event == 'repeat_expansion' or adjs[0].event == 'repeat_contraction':
                    new_copy_nums = set([int(adj.copy_num_change[1]) for adj in adjs])
                    merged_adj.copy_num_change = adjs[0].copy_num_change[0], '/'.join(map(str, sorted(new_copy_nums)))
