...
def find_chimera(aligns, query_seq=None, max_splits=3, debug=False):
    ...
    if path:
        ...
        for i in range(len(chimeric_aligns) - 1):
            adj = call_event(chimeric_aligns[i],
                             chimeric_aligns[i + 1],
                             query_seq=query_seq,
                             no_sort=True)
            if adj is not None:
                if debug:
                    print(
                        'chimera1',
                        chimeric_aligns[i].query,
                        chimeric_aligns[i].target,
                        chimeric_aligns[i].tstart,
                        chimeric_aligns[i].tend,
                        chimeric_aligns[i].qstart,
                        chimeric_aligns[i].qend,
                        chimeric_aligns[i].strand)
                    print('chimera2',
                          chimeric_aligns[i + 1].query,
                          chimeric_aligns[i + 1].target,
                          chimeric_aligns[i + 1].tstart,
                          chimeric_aligns[i + 1].tend,
                          chimeric_aligns[i + 1].qstart,
                          chimeric_aligns[i + 1].qend,
                          chimeric_aligns[i + 1].strand)

                # check for inverted duplication
                check_inv_dup(
                    adj, (chimeric_aligns[i], chimeric_aligns[i + 1]))
