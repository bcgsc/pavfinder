...
class ExonMapper:
    ...
    def sort_adjs(cls, adjs):
        ...
        for i in range(len(adjs)):
            adj_key = adjs[i].chroms[0], adjs[i].genome_breaks[0], adjs[i].genome_breaks[1]
            dict_for_sorting[adj_key] = i

        adjs_sorted = []
        for key in sorted(dict_for_sorting.keys(), key=cmp_to_key(Adjacency.cmp_genome_coords)):
    ...
    def output_events(cls, events, out_file, header=None):
        def create_bedpe_header():
            ...
            return cols + list(report_items.keys())
    ...
    def pick_best_mapping(self, mappings, align):
        ...
        metrics = {}
        for tid, matches in mappings.items():
            transcript = self.transcripts_dict[tid]
            metric = {}
            # points are scored for matching exon boundaries
