...
class Transcript:
    ...
    def get_sequence(self, fasta, cds_only=False, genomic=False):
        ...
        if genomic:
            return fasta.fetch(
                self.chrom, self.exons[0][0] - 1, self.exons[-1][1])
    ...
    def txt_coord_to_exon(self, coord):
        ...
        if type(coord) is int:
            exon_start = 1
            indexes = list(range(len(self.exons)))
            if self.strand == '-':
                indexes.reverse()
            for i in range(len(indexes)):
                if i > 0:
                    exon_start += self.get_exon_size(indexes[i - 1])
                if coord >= exon_start and coord <= exon_start + \
                    self.get_exon_size(indexes[i]) - 1:
    ...
    def get_cds(self):
        cds = []
        if self.is_coding() and self.cds_start is not None and self.cds_end is not None:
            cds_start, cds_end = sorted(
                [int(self.cds_start), int(self.cds_end)])
            ...
            for exon in self.exons:
                if cds_start > exon[1]:
                    ...
                    cds.append(
                        [max(cds_start, exon[0]), min(cds_end, exon[1])])
                elif cds_end < exon[0]:
                    break
                else:
                    cds.append(exon)
    ...
    def extract_transcripts(annotation_file):
        ...
        for feature in BedTool(annotation_file):
            if feature[2] == 'exon':
                ...
                elif 'gene_id' in feature.attrs:
                    gene = feature.attrs['gene_id']
                strand = feature.strand

                coding = None
                if 'gene_biotype' in feature.attrs:
                ...
                try:
                    ...
                    transcript = Transcript(
                        transcript_id, gene=gene, strand=strand, chrom=feature.chrom)
